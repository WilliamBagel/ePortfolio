...
import { CharacterMechanics } from './CharacterMechanics.js';
...
import { KeyButton, PlaneFrame } from "../CSS3DIntegratedRenderer.js";
...
class GameLoader {
  ...
  initCharacter(folder) {
    ...
    // if (folder.model.search("_character") != -1) {
    //   //INIT RAT BOTS!!!
    //   const charMechs = new CharacterMechanics(this, this.character);
    //   this.world.addSystem(charMechs);

    //   this.character.CharacterMechanics = charMechs;
    // }
  }
  ...
  replaceShaderCode(shader, variable, value) {
    ...
    while (newShader != shader || first) {
      ...
      let start = "<" + variable + ">";
      let end = "<\/" + variable + ">";
      let reg = new RegExp("(" + start + "\\*\/)([^\w+])(\/\\*" + end + ")");///<hullNumber>\/\*\w+\*\/<\/hullNumber>"/;
      // reg[Symbol.match] = false;
      // str.replace(prop,"")
      // console.log(reg, reg.exec(shader));
      newShader = shader.replace(reg, "$1" + value+"+1-1" + "$3");
  ...
  _loadObject(obj) {
    ...
    switch (obj.type.toLowerCase()) {
      case 'sync-import': {
        ...
        if(tmp.mesh.getObjectByProperty("type","SkinnedMesh")){
          tmp.mesh.getObjectByProperty("type","SkinnedMesh").frustumCulled = false;
        }
        if (tmp.mesh) mesh = tmp.mesh;
        if (tmp.body) body = tmp.body;
      } break;
      case 'import': {
        mesh = this.loadModel(obj, false).mesh;
        // mesh.geometry.applyQuaternion(new THREE.Quaternion().setFromEuler(new THREE.Euler(Math.PI / 2, 0, Math.PI / 2)));
        this.scene.add(mesh);
        // if (obj.parent) {
        ...
        //   new SyncedMesh(mesh, parent, true, true);
        // }
      } break;
      case 'sync-default':
        if (obj.shape == 'box') {
      ...
      case "checkpoint": {
        ...
        const mat = this.materials.three[obj.shader];
        mesh = new THREE.Mesh(geo, mat);
        // if(obj.size == undefined)console.log(obj)
        mesh.scale.set(obj.size.x, obj.size.y, obj.size.z);
        mesh.position.set(obj.position.x, obj.position.y, obj.position.z);
        obj.rotation = obj.rotation ? obj.rotation : [0, 0, 0];
        mesh.quaternion.copy(new THREE.Quaternion().setFromEuler(new THREE.Euler(THREE.MathUtils.degToRad(obj.rotation[0]), THREE.MathUtils.degToRad(obj.rotation[1]), THREE.MathUtils.degToRad(obj.rotation[2]), 'XYZ')));
        ...
        new SyncedMesh(mesh, body, true, false);
      ...
      case 'planeframe':
        // console.log(obj)
        // const geo = new THREE.PlaneGeometry(obj.size.x,obj.size.y);
        mesh = new PlaneFrame(obj.source, obj.width || 600,obj.height || 600)
        // console.log(obj.size)
        // console.log(obj.size.x||1, obj.size.z||1,0.01)
        mesh.scale.set(obj.size.x||0.01, obj.size.z||0.01, obj.size.y||0.01);
        mesh.position.set(obj.position.x, obj.position.y, obj.position.z);
        // obj.rotation = obj.rotation ? obj.rotation : [0, 0, 0];
        mesh.quaternion.copy(new THREE.Quaternion().setFromEuler(new THREE.Euler(THREE.MathUtils.degToRad(obj.rotation[0]), THREE.MathUtils.degToRad(obj.rotation[1]), THREE.MathUtils.degToRad(obj.rotation[2]), 'XYZ')));
        mesh.quaternion.multiplyQuaternions(mesh.quaternion,new THREE.Quaternion().setFromEuler(new THREE.Euler(THREE.MathUtils.degToRad(90), THREE.MathUtils.degToRad(0), THREE.MathUtils.degToRad(0), 'XYZ')));
      ...
      case 'button':
        ...
        // }
        // mesh._type = "Button";
        // mesh._gameEvent = obj.buttonid
        const button = new KeyButton("./assets/button_e.png","e")
        mesh = button;
        const myInfo = obj
        button.callback = (event)=>{
          if(!button.selectedButton)return;
          this._gameEvents[myInfo.buttonid] && this._gameEvents[myInfo.buttonid].callback(button)
        }
        mesh.range = obj.range || 2.5
        mesh.scale.set(obj.size.x||0.01, obj.size.z||0.01, obj.size.y||0.01);
        mesh.position.set(obj.position.x, obj.position.y, obj.position.z);
        ...
        this._keyButtons.push(mesh);

        mesh.boundobject = this._fetchObject(obj.boundobject)
        mesh.buttonSelectable = true;
    ...
    if(mesh && body && this.debug){
      const mesh2 = new THREE.Mesh(new THREE.BoxGeometry(), Game.materials.three.default);
      mesh2.scale.copy(mesh.scale);
      new SyncedMesh(mesh2,body);
      this.scene.add(mesh2)
    }
    if (obj.parent && mesh) {
      const parent = this._fetchObject(obj.parent);
      if (parent == undefined) console.error("Parent with uuid " + obj.parent + " does not exist");
      // const scale = mesh.scale.clone()
      // mesh.scale.set(1,1,1)
      mesh.updateMatrix()
      // console.log(new THREE.Vector3().setFromMatrixPosition(mesh.matrix))
      // console.log(new THREE.Euler().setFromRotationMatrix(mesh.matrix))
      // console.log(new THREE.Vector3().setFromMatrixPosition(parent.mesh.matrixWorld))
      // console.log(new THREE.Euler().setFromRotationMatrix(parent.mesh.matrixWorld))
      parent.mesh.updateMatrixWorld()
      // const newMat = new THREE.Matrix4().multiplyMatrices(parent.mesh.matrixWorld.invert(),mesh.matrix)
      // console.log(new THREE.Vector3().setFromMatrixPosition(newMat))
      // console.log(new THREE.Euler().setFromRotationMatrix(newMat))
      // mesh.position.setFromMatrixPosition(newMat);
      // mesh.rotation.setFromRotationMatrix(newMat);
      // mesh.position.sub(parent.mesh.position)
      // mesh.scale.copy(scale)
      // parent.mesh.add(mesh);
      // this.scene.add(mesh)
      // new SyncedMesh(mesh,parent.mesh);
      // const parent = this._fetchObject(obj.parent).body;
        //   if (parent == undefined) console.error("Parent with uuid " + obj.parent + " does not exist");
        //   new SyncedMesh(mesh, parent, true, true);
      // const empty = new THREE.Object3D();
      // new SyncedMesh(mesh,parent.mesh,true,true);
      parent.mesh.attach(mesh);
      // empty.attach(mesh);
      // this.scene.add(mesh);
    }
    if (mesh && mesh.animations && Object.keys(mesh.animations).length > 0){
      mesh.actions = {}
      mesh.animMixer = new THREE.AnimationMixer( mesh );
      for (let name in mesh.animations){
        mesh.actions[name] = mesh.animMixer.clipAction(mesh.animations[name]);
      }
      this._meshAnimMixers.push(mesh.animMixer);
    }
  ...
  _fetchObject(uuid) {
    ...
    loadedObj.name = uuid;
    if(loadedObj.mesh)loadedObj.mesh.name = uuid;
  ...
  initCore() {
    ...
    light.shadow.mapSize.width = 1012//2560;
    light.shadow.mapSize.height = 1012//2560;
    ...
    this.mouseCastInfo = undefined;
    this.selectedObject = undefined;

    let buttonsInRange = [];
    this.onrender = (ms)=>{
      for(let mixer of this._meshAnimMixers){
        mixer.update(ms);
      }
    }
    this.onrender = ()=>{
      buttonsInRange = [];
      // console.log(this._keyButtons);
      for(let button of this._keyButtons){
        const buttonPosition = new THREE.Vector3().setFromMatrixPosition(button.matrixWorld)
        const distance = buttonPosition.sub(this.character.mesh.position).length();
        if(distance > button.range){
        ...
        button.opacity = 0.25;
        button.transparency
        buttonsInRange.push({distance,button});
        button.lookAt(this.camera.position);
      }
      buttonsInRange.sort((info1,info2)=>{
        return info1.distance > info2.distance?1:-1;
      })
      let button;
      for(let i in buttonsInRange){
        button = buttonsInRange[i].button;
        if(button.buttonSelectable == false)continue;
        button.opacity = 1;
    ...
    // this.world._clickHandlers.push((event)=>{
    //   this.mouseRaycaster.setFromCamera(this.world.renderer.mousePosition,this.camera);
    //   this.mouseCastInfo = this.mouseRaycaster.intersectObjects(this.scene.children,true);
    //   this.selectedObject = this.mouseCastInfo.length > 0 && this.mouseCastInfo[0].object;
    //   if(!this.selectedObject || (this.selectedObject && this.selectedObject._type != "Button"))return;
    //   const button = this.selectedObject;
    //   this._gameEvents[button._gameEvent] && this._gameEvents[button._gameEvent].callback(button);
    // })        
  }
  addGameEvents(events){
    for(let name in events){
      this._gameEvents[name] = events[name];
    }
  }
}
...
class SyncedMesh {
  ...
  sync(direction, deltatime) {
    ...
    if (this.onSync) {
      ...
      const parent = this.ThreeMesh.parent
      // this.ThreeMesh.matrix.makeScale(1,1,1);
      // Game.scene.attach(this.ThreeMesh);
      if (direction) {
        // console.log(this.CannonMesh.position.y)
        // if(this.CannonMesh.position == undefined)console.log(this)
        // this.ThreeMesh.matrixWorld.decompose(this.CannonMesh.position,this.quaternionSync?this.CannonMesh.quaternion:new THREE.Quaternion(),new THREE.Vector3())
        const pos = new THREE.Vector3().copy(this.CannonMesh.position)
        // this.ThreeMesh.position.copy(this.ThreeMesh.worldToLocal(pos));
        ...
        this.ThreeMesh.matrixWorld.decompose(pos,quat,new THREE.Vector3())
        this.CannonMesh.position.copy(pos);
        if (this.quaternionSync) this.CannonMesh.quaternion.copy(quat);
        
      }
      // parent.attach(this.ThreeMesh);
    }
  }
}
